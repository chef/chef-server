...
add_command "chef12-upgrade-data-transform", "Transfrom data from an open source Chef 11 server for upload to an Chef 12 server.", 2 do
   ...
   def parse(args)
    ...
    opt_parser = OptionParser.new do |opts|
      ...
      opts.on("-d", "--chef11-data-dir [directory]", "Directory of open source Chef 11 server data. (Will ask interactively if not passed)") do |chef11_dir|
        @options.chef11_data_dir = chef11_dir
      end

      opts.on("-e", "--chef12-data-dir [directory]", "Directory to place transformed data. Defaults to a tmp dir.") do |chef12_dir|
