...
module "backend1" {
  ...
  name              = "backend1-${var.scenario}-${var.enable_ipv6 == "true" ? "ipv6" : "ipv4"}-${var.platform}"
}
...
module "backend2" {
  ...
  build_prefix      = var.build_prefix
  capture_paths     = ["/etc/chef-backend", "/var/log/chef-backend"]
  name              = "backend2-${var.scenario}-${var.enable_ipv6 == "true" ? "ipv6" : "ipv4"}-${var.platform}"
}
...
module "backend3" {
  ...
  build_prefix      = var.build_prefix
  capture_paths     = ["/etc/chef-backend", "/var/log/chef-backend"]
  name              = "backend3-${var.scenario}-${var.enable_ipv6 == "true" ? "ipv6" : "ipv4"}-${var.platform}"
}
...
resource "null_resource" "chef_backend_test" {
  ...
  provisioner "file" {
    source      = "files/test_chef_backend-demotion.sh"
    destination = "/tmp/test_chef_backend-demotion.sh"
  }

  # test leader demotion
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/test_chef_backend-demotion.sh",
      "ENABLE_CHEF_BACKEND_DEMOTION=${var.enable_chef_backend_demotion} /tmp/test_chef_backend-demotion.sh",
    ]
  }
}
