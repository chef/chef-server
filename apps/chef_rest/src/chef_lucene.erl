...
'field'(Input, Index) ->
  p(Input, Index, 'field', fun(I,D) -> (p_seq([p_label('name', fun 'field_name'/2), p_string(<<":">>), p_label('arg', p_choose([fun 'term'/2, fun 'group'/2, fun 'string'/2]))]))(I,D) end, fun(Node, Idx) -> transform('field', Node, Idx) end).
...
'range_entry'(Input, Index) ->
  p(Input, Index, 'range_entry', fun(I,D) -> (p_choose([p_one_or_more(fun 'valid_letter'/2), p_string(<<"*">>)]))(I,D) end, fun(Node, Idx) -> transform('range_entry', Node, Idx) end).
...
transform(Symbol,Node,Index) -> lucene_txfm:transform(Symbol, Node, Index).
