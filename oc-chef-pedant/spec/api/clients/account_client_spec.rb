...
describe "opscode-account endpoint", :clients do
  ...
  context "Client ACLs", :acls do
    context "the initial validator client" do
      ...
      it "has the appropriate ACLs" do
        get(api_url.call("/clients/#{client}/_acl"), platform.admin_user).should look_like({
            status: 200,
            body_exact: {
              "create" => {
                "actors" => ["pivotal"],
    ...
    context "a new validator client" do
      ...
      before :each do
        post(api_url.call("/clients"), requestor,
          payload: { "name" => client, "validator" => true }).should have_status_code(201)
      end

      after :each do
        delete(api_url.call("/clients/#{client}"), requestor)
      end
  ...
  context "Admin User Group", :groups do
    ...
    it "has the appropriate members" do
      get(api_url.call("/groups/admins"), platform.superuser).should look_like({
          status: 200,
          body_exact: {
  ...
  context "Client Group", :groups do
    ...
    context "group retrieval" do
      let(:request_url) { api_url.call("groups/#{group_name}") }
    ...
    context "group ACL retrieval" do
      let(:request_url) { api_url.call("groups/#{group_name}/_acl") }
  ...
  context "Client Container", :containers do
    ...
    it "retrieves the Clients container" do
      get(api_url.call("/containers/clients"), requestor).should look_like({
          status: 200,
          body_exact: {
    ...
    it "retrieves the Clients container's ACL" do
      ...
      get(api_url.call("/containers/clients/_acl"), requestor).should look_like({
          status: 200,
          body_exact: {
            "create" => {
  ...
  context "Client Creation" do
    ...
    after :each do
      delete(api_url.call("/clients/#{new_client_name}"), platform.admin_user)
    end
    ...
    context "by a validator" do
      ...
      it "creates a new non-validator client" do
        post(api_url.call("/clients"), requestor,
          payload: { "name" => new_client_name }).should look_like({
            status: 201,
            body: {
              "uri" => api_url.call("/clients/#{new_client_name}"),
            },
          })
      end
      ...
      it "has the validator removed from the new client's ACL" do
        post(api_url.call("/clients"), requestor, payload: { "name" => new_client_name }).should have_status_code(201)

        get(api_url.call("/clients/#{new_client_name}/_acl"), platform.admin_user).should look_like({
            status: 200,
            body_exact: {
              "create" =>
      ...
      it "puts the new client into the 'clients' group" do
        ...
        get(api_url.call("/groups/clients"), platform.admin_user).should look_like({ status: 200,
            body_exact: {
              "name" => "clients",
              "groupname" => "clients",
              "orgname" => platform.test_org.name,
