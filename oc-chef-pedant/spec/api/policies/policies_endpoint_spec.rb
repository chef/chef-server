...
describe "/policies API endpoint", :policies, :policy_name do
  ...
  context "when no policies or policy_groups exist on the server" do
    ...
    it "GET /policies returns an empty list" do
  ...
  context "when a policy assigned to a group exists" do
    ...
    it "GET /policies returns a data structure with the groups assigned policy revision" do
      expect(response.code).to eq(200)
      response_obj = parse(response.body)
      expect(response_obj).to be_a_kind_of(Hash)
  ...
  context "policy exists but has been removed from all policy groups" do
    ...
    it "GET /policies shows the policy" do
      expect(response.code).to eq(200)
      response_obj = parse(response.body)
      expect(response_obj).to be_a_kind_of(Hash)
      expect(response_obj).to eq(expected_response_data)
  ...
  context "with multiple policies and groups" do
    ...
    it "GET /policies returns a data structure with all the policies" do
      expect(response.code).to eq(200)
      response_obj = parse(response.body)
      expect(response_obj).to be_a_kind_of(Hash)
      expect(response_obj).to eq(expected_response_data)
