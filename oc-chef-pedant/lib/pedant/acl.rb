module Pedant
  module ACL
    ...
    def restrict_permissions_to(acl_base, permissions = {})
      permissions[superuser] = %w{create read update delete grant} unless permissions[superuser]
      @original_permissions ||= {}
      @original_permissions[acl_base] = get(api_url.call("#{acl_base}/_acl"), superuser)
      %w{create read update delete grant}.each do |type|
        users = permissions.keys.select { |user| permissions[user].include?(type) }.map(&:name)
        response = put(api_url.call("#{acl_base}/_acl/#{type}"), superuser, payload: {
          type => {
            "actors" => users,
            "groups" => [],
          },
