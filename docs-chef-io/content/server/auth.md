...
from the server and copied to the `~/.chef` directory manually. If a new
private key is required, it can be regenerated from the Chef Infra Server
and copied to the `~/.chef` directory again.
