...
1. Upgrade the leader node with the `--failover` option:
    ...
    Then the system has not upgraded fully. Make sure it is not still the leader, then if it is not
    
    ```bash
    chef-backend-ctl stop elasticsearch && chef-backend-ctl upgrade
    ```
...
1. We strongly recommend rebuilding the "chef" index version to 6.8.23 after the upgrade is complete on both backends and frontends by following these steps [Rebuild Index]({{< relref "#rebuild-the-chef-index" >}})
