class EcPostgres
  ...
  def self.with_connection(node, database = 'template1', opts = {})
    ...
    max_retries = retries
    begin
      connection = PG::Connection.open('user' => postgres['db_connection_superuser'] || postgres['db_superuser'], 'host' => postgres['vip'], 'password' => postgres['db_superuser_password'], 'port' => postgres['port'], 'sslmode' => postgres['sslmode'], 'dbname' => database)
    rescue => e
      if retries > 0
