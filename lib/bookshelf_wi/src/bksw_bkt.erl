...
resource_exists(#http_req{host = [Bucket | _]} = Rq,
                #req_state{dir = Dir} = St) ->
    {(?BACKEND):bucket_exists(Dir, Bucket), Rq, St}.

delete_resource(#http_req{host = [Bucket | _]} = Rq,
                #req_state{dir = Dir} = St) ->
...
create_resource(#http_req{host = [Bucket | _]} = Rq,
                #req_state{dir = Dir} = St) ->
...
to_xml(#http_req{host = [Bucket | _]} = Rq,
       #req_state{dir = Dir} = St) ->
...
content_types_accepted_test_() ->
    [{"should only support PUT with 'undefined' "
      "(absent) Content-Type",
      fun () ->
              {Types, _, _} = content_types_accepted(#http_req{},
                                                     #req_state{}),
...
resource_exists_test_() ->
    [{"should only return true if the bucket "
      "exists",
      fun () ->
              {Dir, Bucket} = test_bucket(),
              ?assertMatch({true, _, _},
                           (resource_exists(#http_req{host = [Bucket]},
                                            #req_state{dir = Dir}))),
              ?assertMatch({false, _, _},
                           (resource_exists(#http_req{host = [<<"batman!">>]},
                                            #req_state{dir = Dir})))
      end}].

delete_resource_test_() ->
    [{"should delete the bucket and if the "
      "bucket exists",
      fun () ->
              {Dir, Bucket} = test_bucket(),
              {true, _, _} = delete_resource(#http_req{host =
                                                           [Bucket]},
                                             #req_state{dir = Dir}),
              {false, _, _} = delete_resource(#http_req{host =
                                                            [<<"derp">>]},
                                              #req_state{dir = Dir})
      end}].

create_resource_test_() ->
    [{"should create the bucket only if the "
      "bucket doesn't exist",
      fun () ->
              {Dir, Bucket} = test_bucket(),
              {false, _, _} = create_resource(#http_req{host =
                                                            [Bucket]},
                                              #req_state{dir = Dir}),
              {true, _, _} = create_resource(#http_req{host =
                                                           [<<"hurp">>]},
                                             #req_state{dir = Dir})
      end}].
...
test_bucket() ->
    ...
    Dir = filename:join("/tmp",
                        io_lib:format("~p~p~p", [Ma, Se, Mi])),
    file:make_dir(Dir),
    Bucket = io_lib:format("~p~p~p", [Ma, Se, Mi]),
    ?assertEqual(ok,
                 (bookshelf_fs:bucket_create(Dir, Bucket))),
    {Dir, Bucket}.
