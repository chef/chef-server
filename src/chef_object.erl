...
new_record(chef_environment, OrgId, AuthzId, EnvData) ->
    ...
    Data = chef_db_compression:compress(chef_environment, jiffy:encode(EnvData)),
...
new_record(chef_data_bag_item, OrgId, _AuthzId, {BagName, ItemData}) ->
    ...
    Data = chef_db_compression:compress(chef_data_bag_item, jiffy:encode(ItemData)),
...
new_record(chef_node, OrgId, AuthzId, NodeData) ->
    ...
    Id = make_org_prefix_id(OrgId, Name),
    Data = chef_db_compression:compress(chef_node, jiffy:encode(NodeData)),
    #chef_node{id = Id,
               authz_id = maybe_stub_authz_id(AuthzId, Id),
               org_id = OrgId,
               name = Name,
...
new_record(chef_role, OrgId, AuthzId, RoleData) ->
    ...
    Id = make_org_prefix_id(OrgId, Name),
    Data = chef_db_compression:compress(chef_role, jiffy:encode(RoleData)),
    #chef_role{id = Id,
               authz_id = maybe_stub_authz_id(AuthzId, Id),
               org_id = OrgId,
               name = Name,
...
new_record(chef_cookbook_version, OrgId, AuthzId, CBVData) ->
    ...
    Deps = jiffy:encode(ej:get({<<"dependencies">>}, Metadata0, {[]})),
...
compress_maybe(Data, Type) ->
    chef_db_compression:compress(Type, jiffy:encode(Data)).
...
update_from_ejson(#chef_data_bag_item{} = DataBagItem, DataBagItemData) ->
    ...
    DataBagItemJson = jiffy:encode(DataBagItemData),
...
update_from_ejson(#chef_node{} = Node, NodeJson) ->
    ...
    Data = chef_db_compression:compress(chef_node, jiffy:encode(NodeJson)),
