...
declare_metrics() ->
    ...
    prometheus_counter:declare([{name, chef_opensearch_search_with_scroll_count},
    ...
    prometheus_counter:declare([{name, chef_opensearch_search_resp_count},
                                ...
                                {labels, [resp_code]}]),
    prometheus_counter:declare([{name, chef_opensearch_search_with_scroll_resp_count},
...
fields_tag() ->
    case envy:get(chef_index, solr_opensearch_major_version, 2, non_neg_integer) of
...
%% Do a search query using the opensearch Scroll API. We only use this when
...
search_with_scroll(#chef_solr_query{} = Query) ->
    prometheus_counter:inc(chef_opensearch_search_with_scroll_count),
    Url = "/chef/_search?scroll=1m",
    {ok, Code, _Head, Body} = chef_index_http:request(Url, get, query_body(Query), ?JSON_HEADER),
    prometheus_counter:inc(chef_opensearch_search_with_scroll_resp_count, [Code]),
    case Code of
        "200" ->
...
delete_ids([]) ->
    ...
    ok;
delete_ids([Id | Ids]) ->
    URI = case envy:get(chef_index, solr_opensearch_major_version, 2, non_neg_integer) of
