-- Update add_user and update_user functions to handle disabled field
-- requires: users_add_disabled
-- requires: create_and_update_users
...
CREATE OR REPLACE FUNCTION add_user(p_id users.id%TYPE,
                                    ...
                                    p_disabled users.disabled%TYPE) RETURNS integer AS $$
  DECLARE
    ...
    WITH createduser AS
      (INSERT INTO users
       ...
       VALUES (p_id, p_authz_id, p_username, p_email, 'this_is_not_a_key',
         ...
         p_serialized_object, p_admin, p_pubkey_version, p_disabled) RETURNING 1)
...
CREATE OR REPLACE FUNCTION update_user(p_pubkey_version users.pubkey_version%TYPE,
                                       ...
                                       p_updated_at users.updated_at%TYPE,
                                       p_admin users.admin%TYPE,
                                       p_disabled users.disabled%TYPE,
