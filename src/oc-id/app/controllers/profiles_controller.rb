...
class ProfilesController < ApplicationController
  ...
  def update
    @user = current_user
    updated_email = (permit_all_params.key? :email) && (permit_all_params[:email] != @user.email)
  ...
  def regen_key
    ...
    send_data @user.private_key, type: 'application/pem-keys', disposition: 'attachment',
                                 filename: "#{@user.username}.pem"
  end
  ...
  def valid_signature?
    return false unless %i[signature user expires].all? { |p| params[p].present? }
  ...
  def user_params
    params.require(:user).permit(:username, :first_name, :last_name, :middle_name, :public_key, :private_key,
