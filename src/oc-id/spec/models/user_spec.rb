...
describe User do
  ...
  it 'should update users on the server' do
    ...
    expect(chef).to receive(:put_rest).with('users/jimmeh',
                                            new_attrs.stringify_keys).and_return({ 'private key' => 'my awesome private key' })
  ...
  describe 'updating a password' do
    fields = %i[current_password new_password password_confirmation]
    fields.each do |f|
      it "populates an error if #{f} is blank" do
        params = fields.reject { |x| x == f }.each_with_object({}) do |field, result|
          result[field] = 'haha'
        end
