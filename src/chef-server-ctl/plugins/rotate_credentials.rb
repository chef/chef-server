...
add_command_under_category "rotate-credentials", "Secrets Management", "Rotate #{ChefUtils::Dist::Server::PRODUCT} credentials for a given service", 2 do
  ...
  OptionParser.new do |opts|
    opts.banner = "Usage: #{ChefUtils::Dist::Server::SERVER_CTL} rotate-credentials $service_name"
  ...
  begin
    ...
    if status.success?
      ...
      log("Run '#{ChefUtils::Dist::Server::SERVER_CTL} rotate-credentials #{service}' on each #{ChefUtils::Dist::Server::PRODUCT}", :notice)
      exit(0)
    else
...
add_command_under_category "rotate-all-credentials", "Secrets Management", "Rotate all #{ChefUtils::Dist::Server::PRODUCT} service credentials", 2 do
  ...
  begin
    ...
    log("Rotating all #{ChefUtils::Dist::Server::PRODUCT} service credentials...", :notice)
  ...
  begin
    ...
    if status.success?
      ...
      log("Run '#{ChefUtils::Dist::Server::SERVER_CTL} rotate-all-credentials' on each #{ChefUtils::Dist::Server::PRODUCT}", :notice)
      exit(0)
    else
      log("Credential rotation failed", :error)
      exit(1)
...
add_command_under_category "rotate-shared-secrets", "Secrets Management", "Rotate the #{ChefUtils::Dist::Server::PRODUCT} shared secrets and all service credentials", 2 do
  ...
  begin
    ...
    if status.success?
      ...
      log("Please copy #{secrets_file_path} to each #{ChefUtils::Dist::Server::PRODUCT} and run '#{ChefUtils::Dist::Server::SERVER_CTL} reconfigure'", :notice)
      exit(0)
    else
...
add_command_under_category "require-credential-rotation", "Secrets Management", "Disable the #{ChefUtils::Dist::Server::PRODUCT} and require credential rotation", 2 do
  ...
  OptionParser.new do |opts|
    opts.banner = "Usage: #{ChefUtils::Dist::Server::SERVER_CTL} require-credential-rotation [--yes]"
    ...
    opts.on("-y", "--yes", "Agree to disable the #{ChefUtils::Dist::Server::PRODUCT} and require credential rotation") do
      @agree_to_disable = true
    end
  end.parse!(ARGV)

  # Agree to disable and require rotation
  msg = "Are you sure you want to disable the #{ChefUtils::Dist::Server::PRODUCT} and require credential rotation?" \
          "\n Type 'yes' to confirm: "
  ...
  log("The #{ChefUtils::Dist::Server::PRODUCT} has been disabled until credentials have been rotated. "\
      "Run 'sudo #{ChefUtils::Dist::Server::SERVER_CTL} rotate-shared-secrets' to rotate them.")
...
add_global_pre_hook "require_credential_rotation" do
  ...
  raise("You must rotate the #{ChefUtils::Dist::Server::PRODUCT} credentials to enable the #{ChefUtils::Dist::Server::PRODUCT}. "\
        "Please run 'sudo #{ChefUtils::Dist::Server::SERVER_CTL} rotate-shared-secrets'")
end
