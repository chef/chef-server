...
bulk_actor_is_authorized(ReqId, ActorId, ResourceType, Resources, Permission) ->
    CheckAuthz = fun(RequestFun) ->
                         Results = make_bulk_actor_is_authorized_request(RequestFun, ActorId, Permission,
                                                                         ResourceType, Resources),
                         Results
                 end,
    oc_chef_authz_http:with_new_http_client(ReqId, ActorId, CheckAuthz).

make_bulk_actor_is_authorized_request(RequestFun, RequestorId, Permission, ResourceType, Resources) ->
    Url = <<"bulk">>,
    {AuthzIds, Data} = lists:unzip(Resources),
    ...
    Body = iolist_to_binary(jiffy:encode(make_bulk_request_body(RequestorId, Permission, ResourceType, AuthzIds))),
    case RequestFun(Url, post, [], Body) of
        ok -> true;
        {ok, ResponseBody} ->
            case ej:get({<<"unauthorized">>}, ResponseBody, fail) of
                fail -> {error, unexpected_body};
                Unauthorized ->
                   {false, get_data_for_id(Unauthorized, Resources) }
            end;
        %% This needs a little more thought, I think...
