-module(mover_chef_reindex_worker).

-behaviour(gen_fsm).

%% Comment this out for VIM syntax check/compiles to work.
-compile([{parse_transform, lager_transform}]).

%% API
-export([
    start_link/0,
    reindex_orgs/0,
    reindex_orgs/1,
    reindex_org/1,
    pause/0,
    resume/0,
    cancel/0,
    restore/1
  ]).

%% FSM states
-export([
    ready/2,
    purging/2,
    paused/2,
    reindex_one/2,
    restore_one/2
  ]).
%% gen_fsm callbacks
-export([init/1,
    handle_event/3,
    handle_sync_event/4,
    handle_info/3,
    terminate/3,
    code_change/4]).

-define(SERVER, ?MODULE).
-define(DEFAULT_ORG_MAXIMUM, 100000).

-record(state, {org_names_fun = [], counter = 1, total_count = 0}).

start_link() ->
    gen_fsm:start_link({local, ?SERVER}, ?MODULE, [], []).

reindex_orgs() ->
	reindex_orgs(?DEFAULT_ORG_MAXIMUM).

reindex_orgs(Maximum) ->
	gen_fsm:send_event(?SERVER,
    {start_reindex,
      fun() ->
          moser_state_tracker:next_reindex_ready_org()
      end,
      Maximum}).

reindex_org(Org) ->
  gen_fsm:send_event(?SERVER, {start_reindex, Org}).

pause() ->
  gen_fsm:send_event(?SERVER, pause).

cancel() ->
  gen_fsm:send_event(?SERVER, cancel).

resume() ->
  gen_fsm:send_event(?SERVER, continue).

restore(Org) when is_list(Org) ->
  restore(list_to_binary(Org));
restore(Org) when is_binary(Org) ->
  gen_fsm:send_event(?SERVER, {restore, Org}).

init([]) ->
    {ok, ready, #state{}}.
%%event
ready({start_reindex, OrgNameFun, Maximum}, State) ->
    gen_fsm:send_event(self(), reindex),
    {next_state, purging, State#state{org_names_fun = OrgNameFun, total_count = Maximum}};
ready({start_reindex, Org}, State) ->
    gen_fsm:send_event(self(), {reindex_one, Org}),
    {next_state, reindex_one, State};
ready({restore, Org}, State) ->
    gen_fsm:send_event(self(), {restore, Org}),
    {next_state, restore_one, State};
ready(cancel, _State) ->
    {next_state, ready, #state{}};
ready(pause, State) ->
    {next_state, ready, State};
ready(continue, State) ->
    {next_state, ready, State}.

reindex_one({reindex_one, Org}, State) ->
    ok = moser_state_tracker:reindex_started(Org),
    {Time, _Result} = timer:tc(fun() -> moser_chef_couch_removal:delete_org(Org) end),
    ok = moser_state_tracker:reindex_successful(Org),
    lager:info("[1/1] reindexd in ~p milliseconds ~p", [Time div 1000, Org]),
    lager:info("Finished purging 1 Org", []),
    {next_state, ready, State}.

purging(reindex, #state{counter = TotalCount, total_count = TotalCount}) ->
    lager:info("Finished purging ~p", [TotalCount]),
    {next_state, ready, #state{}};

purging(reindex, State = #state{org_names_fun = OrgNameFun, counter = Counter, total_count = TotalCount}) ->
		NewState = case OrgNameFun() of
			OrgName when is_list(OrgName) orelse is_binary(OrgName) ->
        lager:info("Starting reindex of ~p", [OrgName]),
				do_reindex(OrgName, Counter, TotalCount),
				State#state{counter = Counter + 1};
			_ ->
				State#state{counter = TotalCount}
		end,
    gen_fsm:send_event(self(), reindex),
    {next_state, purging, NewState};
purging(cancel, #state{org_names_fun = Orgs}) ->
    lager:info("purging cancelled, remaining orgs ~p", [Orgs]),
    {next_state, ready, #state{}};
purging({restore, Org}, #state{org_names_fun = Orgs }) ->
    restore_and_log(Org, Orgs),
    {next_state, ready, #state{}};
purging(pause, State) ->
    lager:info("purging paused", []),
    {next_state, paused, State}.

paused(cancel, _State) ->
    lager:info("purging cancelled", []),
    {next_state, ready, #state{}};
paused(continue, State) ->
    lager:info("purging continued", []),
    gen_fsm:send_event(self(), reindex),
    {next_state, purging, State};
paused({restore, Org}, #state{org_names_fun = Orgs }) ->
    restore_and_log(Org, Orgs),
    {next_state, ready, #state{}}.

restore_one({restore, Org}, #state{}) ->
    restore_and_log(Org),
    {next_state, ready, #state{}}.

handle_event(_Event, StateName, State) ->
    {next_state, StateName, State}.

handle_sync_event(_Event, _From, StateName, State) ->
    Reply = ok,
    {reply, Reply, StateName, State}.

handle_info(_Info, StateName, State) ->
    {next_state, StateName, State}.

terminate(_Reason, _StateName, _State) ->
    ok.

code_change(_OldVsn, StateName, State, _Extra) ->
    {ok, StateName, State}.

do_reindex(Org, Counter, TotalCount) ->
		ok = moser_state_tracker:reindex_started(Org),
    %% put org in purging state
		{Time, ok} = timer:tc(fun() -> moser_chef_couch_removal:delete_org(Org) end),
    lager:info("[~p/~p] reindexd in ~p milliseconds ~p", [Counter, TotalCount, Time div 1000, Org]),
    %% put org in deleted state
		ok = moser_state_tracker:reindex_successful(Org).
restore_and_log(Org) ->
    lager:info("purging cancelled by restore request for Org ~p", [Org]),
    restore_and_reset(Org).

restore_and_log(Org, Orgs) ->
    lager:info("purging cancelled by restore request for Org ~p, remaining orgs ~p", [Org, Orgs]),
    restore_and_reset(Org).

restore_and_reset(Org) ->
    ok = case moser_state_tracker:org_state(Org) of
      <<"reindex_started">> ->
        moser_state_tracker:reset_reindex_started_org_to_completed(Org);
      <<"reindex_successful">> ->
        moser_state_tracker:reset_reindex_org_to_completed(Org);
      State ->
        lager:error("Org ~p not in expected state. In state ~p", [Org, State]),
        {error, org_not_state_to_be_reset, State}
    end,
    ok = moser_reindex_backup:restore_org(Org).
