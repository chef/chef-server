...
serialize_node({_}=Node) ->
    serialize_node(jiffy:encode(Node)).
...
couch_json_to_record(chef_node, OrgId, AuthzId, RequesterId, Object) ->
    ...
    Date = sql_date(now),
    <<NodeJs/binary>> = jiffy:encode(cleanup_couch_node_record(Object)),
...
couch_json_to_record(chef_role, OrgId, AuthzId, RequesterId, Object) ->
    ...
    Date = sql_date(now),
    RoleJs = jiffy:encode(cleanup_couch_role_record(Object)),
...
couch_json_to_record(chef_environment, OrgId, AuthzId, RequesterId, Object) ->
    Name = ej:get({<<"name">>}, Object),
    Date = sql_date(now),
    EnvironmentJs = jiffy:encode(cleanup_couch_environment_record(Object)),
...
couch_json_to_record(chef_data_bag_item, OrgId, _AuthzId, RequesterId, Object) ->
    Name = ej:get({<<"name">>}, Object),
    Date = sql_date(now),
    Data_Bag_ItemJs = jiffy:encode(cleanup_couch_data_bag_item_record(Object)),
